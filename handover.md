...
| 27c ✅ | Regenerate the index on `go_live!`, suspension, listing edit and new release (replaces Task 26's send-to-D-store step) | 27b | `app.rb`, `release.rb`, `config/environments/test.rb`, spec | Going live or suspending changes the next index | low |
...
**Done in 27c (this session, code-complete, verified — see below):**
- `App`: a single `after_commit :publish_catalog_index_if_needed, on: :update` enqueues `CatalogIndexPublishJob` when `saved_change_to_listing_status?` (covers `go_live!` and the new `suspend!` below) or when a live-relevant listing field changed — `CATALOG_INDEX_LISTING_FIELDS = %w[name publisher_alias play_package_name publisher_profile_id]`, i.e. exactly the App columns `CatalogIndex::Serializer` actually reads (`slug`/`title` from `name`, `publisher.name` from `publisher_display_name` which reads `publisher_alias`/`publisher_profile`, `package_name`). One callback, not three, since ActiveRecord already coalesces a save's attribute changes into one `after_commit`.
- **Added `App#suspend!`** — the other half of `go_live!`'s transition (`live -> suspended`). Nothing called it before this: the `suspended` enum value and `go_live!`'s `listing_suspended? ->` resume path already existed, but there was no way in, only a way back out. No admin UI wired to it yet (there isn't one in this repo — flagged, not built here, see below); this leaf only needed the model method the acceptance check's "suspending changes the next index" hangs off of.
- **New release → publish:** `Release` gets an `after_create :publish_catalog_index_if_app_live`, same style as the other three `after_create` hooks already on that model. Only fires when `app.listing_live?` — a release on a draft/awaiting_payment/suspended app changes nothing `CatalogIndex::Serializer.for_live_apps` (scoped to `App.listing_live`) would show, so publishing for it would be a wasted round trip.
- **`config/environments/test.rb` fix, needed for the new spec:** `config.active_job.queue_adapter` was never set for tests (`config/application.rb` sets the real one to `:good_job`, which has no in-memory `enqueued_jobs` array), so `spec/models/app_spec.rb` — the first spec in this repo to use rspec-rails' `have_enqueued_job` matcher — would have called a method the good_job adapter doesn't implement. Added `config.active_job.queue_adapter = :test`, the standard adapter every Rails app scaffolds by default; this file was just missing it. Doesn't touch the real (`good_job`) adapter used outside tests.
- **`spec/models/app_spec.rb` added** (no `app_spec.rb` existed before this leaf): 8 examples — `go_live!` and `suspend!` each enqueue; `suspend!` on a non-live app returns `false` and doesn't change state; a live app's rename/`publisher_alias`/`play_package_name` edit each enqueue; an unrelated field (`archived`) does not; plain `create` does not.

**Verified how (27c):** same posture as 27b-ii/27b-iii — no Rails/DB/rspec in-sandbox. `ruby -c` on all four changed/added files (`app.rb`, `release.rb`, `config/environments/test.rb`, `spec/models/app_spec.rb`) — syntax OK. Beyond that, a throwaway harness (not committed) reimplemented `publish_catalog_index_if_needed`'s and `publish_catalog_index_if_app_live`'s exact branching against fake objects answering the same predicate methods (`saved_change_to_attribute?`, `saved_change_to_listing_status?`, `listing_live?`) — 10 cases, all passing: `go_live!` enqueues, `suspend!` enqueues, each of the four watched fields enqueues on its own, an unrelated field does not, no change does not, a release on a live app enqueues, a release on a non-live app does not. **Not verified:** the actual committed `app_spec.rb` has not been run against a real database (no Postgres in-sandbox either), so the `have_enqueued_job` wiring and the FactoryBot `:app` factory interaction are unverified beyond the syntax check and the harness's logic-level mirror.

**Flagged, not fixed here:**
- **No debouncing**, exactly the gap 27b-iii's own note already called out for this leaf ("27c should debounce enqueues (`set(wait:)`) so a burst of edits makes one publish"). A script that updates many apps, or several releases landing close together, enqueues one `CatalogIndexPublishJob` per triggering save today — each one regenerates the *current* full state (per that job's own comment, "the index is regenerated from current state each time, so a retry can never publish something stale"), so this is wasted publish volume, not a correctness bug, but it's real load against the GitHub Pages publish path and its advisory lock. Left for a follow-up rather than building `set(wait:)`/GoodJob-concurrency debouncing into this leaf, which was already the case 27b-iii deferred it to begin with.
- **No admin suspension UI.** `App#suspend!` exists and is exercised by this leaf's spec, but nothing in the app calls it — no admin controller action, no view. Out of scope for 27c itself (predicted files were `app.rb`, one job — this leaf is the model-level hook the eventual UI will call, not the UI).
- **Listing-edit coverage is currently name/alias/package-name/profile only**, matching what the *current* serializer reads. Task 30 (staged listing edits — description, icon, screenshots, category, etc., all still `reserved` in `catalog_index_v2.md`) will add more columns the index reads; each will need adding to `CATALOG_INDEX_LISTING_FIELDS` when it lands, or this hook will silently stop covering a real listing edit.
- Per this file's own convention (`27b-iii`'s note): none of this was run against the real GitHub Pages repo, a real Postgres advisory lock, or GoodJob's actual queue — only against fakes/harnesses, same limitation already logged for the pipeline this leaf hooks into.

27c closes out the smallest set (27a–27c) that replaces Task 26's old send-to-D-store step — **an app going live, being suspended, having its listing edited, or getting a new release now actually enqueues a catalog index republish**, not just "unblocked by code" as the line below used to say. 31a (D-Store's `5.g.v.zi` blocker) still needs its own admin/model/serializer work on top of this — this leaf only wires the *publish trigger*, not the editorial-authoring UI 31a is about.
